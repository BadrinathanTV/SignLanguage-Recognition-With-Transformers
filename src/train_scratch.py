...
if __name__ == '__main__': 
    ...
    # COMMENTED OUT FOR SCRATCH TRAINING
    ...
    logger.print_table("🏋️ Scratch Training Configuration", list(training_config.keys()), [list(training_config.values())])
    ...
    with rich_training_context() as training_handler:
        for epoch in range(epochs): 
            with training_handler.create_training_progress() as epoch_progress:
                epoch_task = epoch_progress.add_task(f"[bold red] Scratch Progress {epoch+1}/{epochs}", train_loss=0.0, test_loss=0.0, total=train_batches)
                # Training phase
                model.train()
                ...
                with torch.no_grad():
                    for batch_idx, batch in enumerate(test_dataloader):
                        ...
                        test_loss_avg = round(test_epoch_loss/test_batches,5) if test_batches > 0 else 0.0
                        epoch_progress.update(epoch_task, advance=0, test_loss=test_loss_avg)
                
                # Save checkpoints
                if epoch % 10 == 0 and epoch != 0: 
                    checkpoint_path = f"experiment/checkpoints/scratch_{epoch}_model.pt"
                    save(model.state_dict(), checkpoint_path)
                    training_handler.save_checkpoint_status(checkpoint_path, epoch)
            
    # Final save
    save(model.state_dict(), f"experiment/checkpoints/scratch_{epoch}_model.pt")
