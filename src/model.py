...
class DETR(nn.Module):
    ...
    def load_pretrained(self, checkpoint_path: str):
        ...
        try:
            ...
            # Warn if some keys were excluded
            if len(pretrained_dict) < len(state_dict):
                skipped_keys = set(state_dict.keys()) - set(pretrained_dict.keys())
                self.logger.warning(f"Skipping loading weights for layers with mismatched shapes: {skipped_keys}")
